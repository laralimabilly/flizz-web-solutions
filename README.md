...
### Traditional Hosting (Hostinger, current setup)
```bash
npm run build
# Upload dist/ folder to your server
```

The contact form posts to `public/api/contact.php`, which relays through the
Resend API and requires PHP on the host. One-time setup:

1. Copy `public/api/resend-config.example.php` to `public/api/resend-config.php`
   and fill in the Resend API key (the file is gitignored).
2. Build and upload `dist/` as usual; the config ships inside `dist/api/`.
   Alternatively upload `resend-config.php` once via hPanel file manager.

Without the config file the endpoint returns an error and the form falls back
to a prefilled mailto link, so the site never hard-breaks.

### Social share images

`public/og-image.jpg` and the per-page cards in `public/og/` are generated by
`scripts/generate-og.mjs`. When service or project titles change, update the
manifest at the top of that script and re-run:
